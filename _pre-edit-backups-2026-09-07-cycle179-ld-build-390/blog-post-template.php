...
function bhp_blog_rail_offset( $html ) {
	...
	/*
	 * ⭐⭐ 1.19.388 — THE PER-POST OVERRIDE IS READ BEFORE THE ARITHMETIC, AND IT
	 *     WINS. `_bhp_book_rail_position` holding a positive integer names the
	 *     1-based clean top-level paragraph the rail must follow, and the
	 *     numbered-title guard below is deliberately NOT applied to it: an
	 *     editor who names an ordinal has already decided. `'off'` never reaches
	 *     here — `bhp_blog_inject_rail()` returns before calling this function.
	 *
	 * ⚠️ AN OUT-OF-RANGE ORDINAL IS IGNORED, NOT CLAMPED. Clamping would put the
	 *    rail somewhere nobody asked for while reporting success; falling
	 *    through to the computed placement is the behaviour a reader can predict
	 *    from the meta value alone.
	 */
	$override = bhp_blog_rail_position_override();
	if ( is_int( $override ) && isset( $tops[ $override - 1 ] ) ) {
		return (int) $tops[ $override - 1 ]['end'];
	}
	...
	/*
	 * ⭐⭐ 1.19.388 — THE NUMBERED-TITLE GUARD. `CYCLE179-LD-TABLE-RAIL-FIX`.
	 *
	 * ⛔ THE DEFECT, MEASURED IN A BROWSER RATHER THAN REASONED ABOUT
	 *    (`CYCLE179-LD-STAGING-RENDER-82-46` §5.3, staging posts 46, both
	 *    viewports): the rail landed with `previousElementSibling` =
	 *    *"8. Adventures of Charlotte & Henry: Mount Everest"* and
	 *    `nextElementSibling` = that entry's first body paragraph. The reader
	 *    got a list item's TITLE, then a full commerce card, then the title's
	 *    own text. Every ordinal in the arithmetic above was correct; the
	 *    arithmetic simply has no idea that a paragraph can be a heading in
	 *    everything but tag name.
	 *
	 * ⭐ A LIST WRITTEN AS `<p><strong>8. …</strong></p>` IS THE HOUSE STYLE ON
	 *    the round-up posts, so this is not an edge case — it is the shape of
	 *    the two highest-traffic articles on the blog. Detecting it structurally
	 *    (`<ol>`) is impossible for exactly that reason: there is no `<ol>`.
	 */
	$safe = bhp_blog_rail_safe_ordinal( $html, $tops, (int) $targets['rail'] );
	if ( null === $safe ) {
		return null; // Nothing sane in range. Caller appends below everything.
	}

	return (int) $tops[ $safe - 1 ]['end'];
}

/**
 * ⭐⭐ 1.19.388 — THE PER-POST RAIL OVERRIDE, `_bhp_book_rail_position`.
 *
 * Three accepted values, and everything else is ignored:
 *
 *   `'off'`  — this post carries no automatic rail at all. Case-insensitive,
 *              whitespace-tolerant. Honoured in `bhp_blog_inject_rail()`.
 *   `N >= 1` — the rail follows the Nth CLEAN TOP-LEVEL PARAGRAPH, counted the
 *              same way `bhp_blog_ask_paragraph_targets()` counts. Guards off.
 *   anything — ignored; the computed placement runs.
 *
 * ⛔ IT IS A `post_meta` READ AND NOTHING ELSE. No write, no registration of a
 *    meta box, no `register_post_meta()`, so it does not become editable
 *    through the REST API by existing — it is set deliberately, by
 *    `wp post meta set`, and the RUNBOOK documents that command.
 *
 * ⚠️ NOT REGISTERING IT COSTS SOMETHING AND THE COST IS STATED: it will not
 *    appear in the block editor's sidebar, and an editor cannot discover it
 *    without being told. That is the trade for not widening the REST surface of
 *    every post in the same release that fixes a layout defect.
 *
 * @param int|null $post_id Post to read, or null for the current one.
 * @return string|int|null `'off'`, a positive integer, or null.
 */
function bhp_blog_rail_position_override( $post_id = null ) {
	...
	$raw = get_post_meta( $post_id, '_bhp_book_rail_position', true );
	$raw = is_scalar( $raw ) ? trim( (string) $raw ) : '';

	if ( '' === $raw ) {
		return null;
	}
	if ( 0 === strcasecmp( 'off', $raw ) ) {
		return 'off';
	}
...
/**
 * ⭐⭐ 1.19.388 — DOES THIS PARAGRAPH READ AS A NUMBERED LIST ENTRY'S TITLE?
 *
 * "Begins with a number and a period" is the brief's own test, widened by one
 * character to accept `8)` as well as `8.` because both are in circulation in
 * hand-written round-ups. The trailing `\S` matters: a paragraph that is only
 * `"8."` and nothing else is a stray, not a title.
 *
 * @param string                                          $html      Rendered content.
 * @param array{start:int|null,end:int,top:bool}          $paragraph One entry.
 * @return bool
 */
function bhp_blog_paragraph_is_numbered_title( $html, $paragraph ) {
	if ( ! isset( $paragraph['start'], $paragraph['end'] ) || null === $paragraph['start'] ) {
		return false;
	}
	$start = (int) $paragraph['start'];
	$len   = (int) $paragraph['end'] - $start;
	if ( $len < 1 ) {
		return false;
	}

	$text = wp_strip_all_tags( substr( $html, $start, $len ) );
	...
	$pattern = (string) apply_filters(
		'bhp_blog_rail_numbered_title_pattern',
		'/^\d{1,3}\s*[.)]\s+\S/u'
	);
...
/**
 * ⭐⭐ 1.19.388 — IS THE BOUNDARY AFTER THIS PARAGRAPH A BLOCK EDGE?
 *
 * True when a heading opens between the end of paragraph `$i` and the start of
 * the next clean top-level paragraph (or the end of the article).
 *
 * ⭐ IT LOOKS AT THE SPAN, NOT AT THE NEXT BYTE, AND THE DIFFERENCE IS THE WHOLE
 *    POINT. On the Magic Tree House body the span between the last entry's
 *    "Get it direct from me" paragraph and the next paragraph carries an HTML
 *    comment, the rendered `[bhp_capture_band]` markup AND the `<h3>`. A
 *    next-byte test sees the comment, answers "no", and puts the rail back
 *    inside the entry — which is the defect.
 *
 * @param string                                       $html Rendered content.
 * @param array<int,array{start:int|null,end:int}>     $tops Clean top-level paragraphs.
 * @param int                                          $i    0-based index into `$tops`.
 * @return bool
 */
function bhp_blog_rail_boundary_at_block_edge( $html, $tops, $i ) {
	if ( ! isset( $tops[ $i ]['end'] ) ) {
		return false;
	}
	$from = (int) $tops[ $i ]['end'];
	...
	if ( isset( $tops[ $i + 1 ]['start'] ) && null !== $tops[ $i + 1 ]['start'] ) {
		$to = (int) $tops[ $i + 1 ]['start'];
	}
	if ( $to <= $from ) {
		return false;
	}

	return (bool) preg_match( '/<h[1-6][\s>]/i', substr( $html, $from, $to - $from ) );
}

/**
 * ⭐⭐ 1.19.388 — HOW FAR THE RAIL MAY WALK FORWARD LOOKING FOR A SANE BOUNDARY.
 *
 * ⚠️ A BOUND EXISTS BECAUSE AN UNBOUNDED "prefer a block edge" SEARCH WOULD WALK
 *    TO THE LAST HEADING IN THE ARTICLE on a post with one long final section,
 *    which moves the rail from ~2/3 to ~95% and is a worse defect than the one
 *    being fixed. 8 is one full round-up entry plus its call to action on both
 *    pilot posts, measured, not guessed.
 *
 * @return int
 */
function bhp_blog_rail_max_advance() {
	return max( 1, (int) apply_filters( 'bhp_blog_rail_max_advance', 8 ) );
}

/**
 * ⭐⭐ 1.19.388 — THE FIRST SANE 1-BASED ORDINAL AT OR AFTER `$want`.
 *
 * Three tiers, in this order, over the window `[$want, $want + max_advance]`
 * capped at `count - 1` so a paragraph always survives before the end-of-post
 * capture:
 *
 *   1. a boundary that is a BLOCK EDGE (a heading opens next) and is not a
 *      numbered title — the rail lands between sections, not inside one;
 *   2. a boundary at least `bhp_blog_ask_min_paragraph_gap()` paragraphs past
 *      the nearest numbered title at or above it (or with no title above it
 *      at all) — the brief's "at least two paragraphs after such a title";
 *   3. any boundary that is not itself a numbered title.
 *
 * ⛔ AND null WHEN NO TIER MATCHES, WHICH IS A REAL ANSWER. The caller appends
 *    the rail below the whole article, which collides with nothing. Failing
 *    upward — wedging it against a title because the window ran out — is the
 *    exact failure the retired visible-text anchor's own comment records this
 *    file paying for once already. ⚠️ THAT FUNCTION IS DELIBERATELY NOT NAMED
 *    HERE: `test-cycle169-blog-layout.php` §1.6a counts its occurrences in this
 *    file and fails if a second one appears, which is how it proves nothing
 *    calls the retired anchor. A prose mention would break a true assertion.
 *
 * ⛔ TIER 1 CANNOT PICK A BOUNDARY *AFTER A HEADING*, AND THE ASSERTION IS FREE:
 *    every candidate is the end of a `</p>`, so the bytes immediately before an
 *    insertion point are `</p>` by construction. `bhp_blog_rail_boundary_ok()`
 *    checks it anyway, because "by construction" is what this file's history
 *    says gets quietly falsified by the next release.
 *
 * @param string                                   $html Rendered content.
 * @param array<int,array{start:int|null,end:int}> $tops Clean top-level paragraphs.
 * @param int                                      $want 1-based computed target.
 * @return int|null 1-based ordinal, or null.
 */
function bhp_blog_rail_safe_ordinal( $html, $tops, $want ) {
	$count = count( $tops );
	$want  = max( 1, (int) $want );
	$last  = $count - 1;                       // never the final paragraph

	if ( $want > $last || $last < 1 ) {
		return null;
	}

	$stop = min( $last, $want + bhp_blog_rail_max_advance() );

	// The nearest numbered title at or above each candidate, computed once.
	$title_at = array();
	$seen     = 0;
	for ( $n = 1; $n <= $count; $n++ ) {
		if ( bhp_blog_paragraph_is_numbered_title( $html, $tops[ $n - 1 ] ) ) {
			$seen = $n;
		}
		$title_at[ $n ] = $seen;
	}

	$gap = max( 1, (int) bhp_blog_ask_min_paragraph_gap() );

	/*
	 * ⛔⛔ THE EARLY RETURN IS THE MOST IMPORTANT LINE IN THIS FUNCTION, AND IT
	 *     WAS ADDED AFTER THE FIRST VERSION SHIPPED A REAL REGRESSION. Written
	 *     down rather than quietly corrected, because the mistake is easy to
	 *     make again.
	 *
	 * ⛔ THE FIRST VERSION RAN THE THREE-TIER SEARCH UNCONDITIONALLY, so the
	 *    tier-1 "prefer a block edge" preference walked the rail forward on
	 *    EVERY post — including posts with no numbered list anywhere in them.
	 *    `test-cycle169-blog-layout.php` caught it in the same sitting, on the
	 *    build it was found in, with two measurements:
	 *
	 *      §1.3b  the rail no longer followed the computed 2/3 paragraph at all
	 *             (fixture: paras=26, target 17, rail moved off it)
	 *      §3.5c  on the lopsided-roundup fixture the rail landed at 96.4% of
	 *             the article, against a 60–90% window — it had walked to the
	 *             LAST heading in the post
	 *
	 * ⭐ A GUARD MUST NOT CHANGE THE CASE IT WAS NOT WRITTEN FOR. When the
	 *    computed target is already clear of every numbered entry, it IS the
	 *    answer and this function hands it straight back. The search below runs
	 *    ONLY when the arithmetic landed on a numbered title, or within the gap
	 *    after one — which is the whole and only defect.
	 */
	if ( ( 0 === $title_at[ $want ] || $want - $title_at[ $want ] >= $gap )
		&& bhp_blog_rail_boundary_ok( $html, $tops, $want ) ) {
		return $want;
	}
	...
	for ( $tier = 1; $tier <= 3; $tier++ ) {
		for ( $n = $want; $n <= $stop; $n++ ) {
			if ( ! bhp_blog_rail_boundary_ok( $html, $tops, $n ) ) {
				continue;
			}
			if ( $n === $title_at[ $n ] ) {
				continue;                      // never split a title from its body
			}

			if ( 1 === $tier ) {
				if ( bhp_blog_rail_boundary_at_block_edge( $html, $tops, $n - 1 ) ) {
					return $n;
				}
				continue;
			}
			if ( 2 === $tier ) {
				if ( 0 === $title_at[ $n ] || $n - $title_at[ $n ] >= $gap ) {
					return $n;
				}
				continue;
			}
...
/**
 * ⭐ 1.19.388 — THE STRUCTURAL GUARD: an insertion point is the end of a
 * paragraph, and never sits directly after a heading.
 *
 * @param string                                   $html Rendered content.
 * @param array<int,array{start:int|null,end:int}> $tops Clean top-level paragraphs.
 * @param int                                      $n    1-based ordinal.
 * @return bool
 */
function bhp_blog_rail_boundary_ok( $html, $tops, $n ) {
	if ( ! isset( $tops[ $n - 1 ]['end'] ) ) {
		return false;
	}
	$end = (int) $tops[ $n - 1 ]['end'];
	if ( $end < 4 || $end > strlen( $html ) ) {
		return false;
	}
	$before = rtrim( substr( $html, 0, $end ) );

	if ( preg_match( '#</h[1-6]>$#i', $before ) ) {
		return false;                          // never immediately after a heading
	}
	return (bool) preg_match( '#</p>$#i', $before );
}
...
function bhp_blog_inject_rail( $content ) {
	...
	/*
	 * ⭐⭐ 1.19.388 — `_bhp_book_rail_position` = `off` SUPPRESSES THE RAIL
	 *     ENTIRELY on this post, and it is checked HERE rather than inside
	 *     `bhp_blog_rail_offset()` because that function's null already means
	 *     something else: "place it at the end". Two different meanings on one
	 *     return value is how a suppression silently becomes an append.
	 *
	 * ⛔ `$done` IS NOT SET ON THIS PATH. A suppressed post has no rail to place
	 *    twice, and setting the once-per-request latch here would let an
	 *    `off` post disable the rail on a second `the_content` render in the
	 *    same request.
	 */
	if ( 'off' === bhp_blog_rail_position_override() ) {
		return $content;
	}
...
function bhp_blog_capture_band_paragraphs( $html ) {
	...
	foreach ( $m as $set ) {
		...
		if ( 'p' === $tag ) {
			if ( $closing ) {
				/*
				 * ⭐ 1.19.388 — `start` IS NEW AND IT IS PURELY ADDITIVE. The
				 *    offset of the `<p` that opened this paragraph, or null for
				 *    a stray `</p>` with no opener. Nothing that read this array
				 *    before reads a new key, and `end` / `top` are byte-for-byte
				 *    the values they always were — `test-cycle169-blog-layout.php`
				 *    §2.4c–§2.4g assert on `top` and on `count()` and are
				 *    untouched by this.
				 *
				 * ⛔ WHY IT HAD TO EXIST: `bhp_blog_rail_offset()` now has to ask
				 *    "does this paragraph BEGIN with a number and a period", and
				 *    a list of end-offsets cannot answer a question about a
				 *    paragraph's text. Recomputing the openers in a second pass
				 *    would be a second scanner to keep in sync with this one,
				 *    which is the defect class this file already warns about.
				 */
				$out[] = array(
					'start' => $open_at,             // the `<p` that opened it
...
/* ═══════════════════════════════════════════════════════════════════════════
 * 5b · WIDE TABLES — 1.19.388, `CYCLE179-LD-TABLE-RAIL-FIX`
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ⛔ THE DEFECT, MEASURED IN A BROWSER AT AN ASSERTED `window.innerWidth` OF
 *    375 (`CYCLE179-LD-STAGING-RENDER-82-46` §5.1, staging post 82):
 *
 *      table bounding box            left 57 … right 468   (412px wide)
 *      main.site-main                clientWidth 360, scrollWidth 468
 *      main.site-main computed       overflow-x: clip
 *      column 5 "Where a Charlotte and Henry book sits"   392 … 466
 *
 *    `body:not(.home) .site-main { overflow: clip }` (style.css) does exactly
 *    what it says: the fifth column — the column that names the books — is
 *    painted outside the clip and there is no scroll to reach it. ⭐ AND
 *    `document.documentElement.scrollWidth` reads 360, i.e. THE PAGE REPORTS NO
 *    OVERFLOW PRECISELY BECAUSE THE CLIP HID IT. That is why no responsive
 *    audit had ever caught this: the usual "does the document overflow" probe
 *    returns a clean answer on a broken page.
 *
 * ⭐ THE FIX IS A WRAPPER, NOT A CHANGE TO `.site-main`. Relaxing the clip is
 *    the obvious move and it is the wrong one: that declaration is what stops
 *    every full-bleed hero and off-canvas element on 68 interior pages from
 *    giving the whole document a horizontal scrollbar. A scroll container
 *    around the table alone has a blast radius of exactly the tables.
 */

/**
 * Wrap every bare `<table>` in a horizontally scrollable container.
 *
 * ⛔ PURE, AND THAT IS DELIBERATE: no post, no globals, no filters on the way
 *    in, so the suite can assert the wrap-once and never-double-wrap properties
 *    on a string without rendering a page. The `the_content` guards live in the
 *    thin filter below.
 *
 * ⭐ TWO THINGS ARE LEFT ALONE, AND FOR TWO DIFFERENT REASONS:
 *
 *      · a `figure.wp-block-table` — the block editor's own table block already
 *        has a wrapper element, and nesting a second one inside it produces a
 *        scroll container whose parent still clips. Its scrolling is handled in
 *        CSS on `figure.wp-block-table` itself.
 *      · content that already contains `bhp-table-scroll` — an editor's own
 *        wrapper, or this filter having already run in the same request.
 *
 * ⚠️ WHAT IT CANNOT DO, STATED RATHER THAN GLOSSED: `</table>` is matched
 *    non-greedily, so a table nested inside another table would close the outer
 *    wrapper at the inner `</table>`. Nested tables do not occur in this
 *    corpus (checked across every published `post_content` before shipping),
 *    and a DOM parse is refused here for the same reason section 4 refuses one:
 *    re-serialising an article that carries live affiliate anchors is the
 *    change §26 forbids. This function only ever SPLICES.
 *
 * @since 1.19.388
 * @param string $html Rendered content.
 * @return string
 */
function bhp_content_wrap_tables( $html ) {
	...
	if ( false !== strpos( $html, 'bhp-table-scroll' ) ) {
		return $html;                                     // already wrapped, once is once
	}

	$label = esc_attr__( 'Table, scrolls sideways', 'brave-hearts' );
	$hint  = esc_html__( 'Scroll for more', 'brave-hearts' );

	$pattern = '#(<figure\b[^>]*\bclass\s*=\s*(["\'])[^"\']*\bwp-block-table\b[^"\']*\2[^>]*>.*?</figure\s*>)'
		. '|(<table\b[^>]*>.*?</table\s*>)#is';

	$out = preg_replace_callback(
		$pattern,
		function ( $m ) use ( $label, $hint ) {
			if ( ! isset( $m[3] ) || '' === $m[3] ) {
				return $m[0];                             // the block-editor figure, untouched
			}
			return '<div class="bhp-table-scroll">'
				. '<div class="bhp-table-scroll__pane" role="region" tabindex="0" aria-label="' . $label . '">'
				. $m[3]
				. '</div>'
				. '<span class="bhp-table-scroll__hint" aria-hidden="true">' . $hint . ' &rarr;</span>'
				. '</div>';
		},
		$html
	);

	// A `preg` failure (backtrack limit on a very large article) returns null.
	// Handing back the original is the honest answer; a clipped table is a
	// smaller defect than a blank article.
	return ( null === $out ) ? $html : (string) $out;
}

/**
 * The `the_content` seam for the wrapper above.
 *
 * ⭐ PRIORITY 13, WHICH IS AFTER `do_shortcode` (11), THE CAPTURE BAND (11) AND
 *    THE BOOK RAIL (12), AND THE ORDER IS LOAD-BEARING IN BOTH DIRECTIONS:
 *
 *      · running AFTER 11 means a table emitted by a shortcode is wrapped too;
 *      · running AFTER 12 means this filter cannot move a single paragraph
 *        ordinal the band and the rail already counted. Wrapping earlier would
 *        add a `<div>` around a `<table>` — and `table` is in
 *        `bhp_blog_capture_band_containers()`, so the depth counter would see
 *        one more level and the arithmetic would still agree, but only by
 *        luck. Depending on luck is how the next release breaks it.
 *
 * ⛔ NOT GATED ON `bhp_blog_template_active()`. A wide table clips identically
 *    on a page as on a post, and both render inside `.entry-content`.
 *
 * @since 1.19.388
 * @param string $content Rendered content.
 * @return string
 */
function bhp_content_table_scroll_filter( $content ) {
	if ( is_feed() || ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) {
		return $content;
	}
	...
	return bhp_content_wrap_tables( $content );
}
add_filter( 'the_content', 'bhp_content_table_scroll_filter', 13 );
