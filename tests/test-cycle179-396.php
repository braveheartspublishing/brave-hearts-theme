...
/*
 * ⛔⛔ CORRECTED 1.19.397 (`CYCLE179-LD-BUILD-397`) — THIS WAS AN EQUALITY ON A
 *     MOVING NUMBER, AND IT FAILED THE FIRST CORRECT BUILD AFTER IT.
 *
 *     ~~'0.5 style.css declares 1.19.396'~~
 *     ~~false !== strpos( …style.css…, 'Version: 1.19.396' )~~
 *
 * ⭐ IT IS THE SAME DEFECT THE RUNBOOK ALREADY CORRECTED IN ITSELF: the
 *    artefact assertion `*.min.css # MUST be 10` was changed to a floor
 *    because "a fixed number goes stale and then gets corrected downward by
 *    someone trusting it", and because a correct build failing a gate teaches
 *    the next builder to distrust a passing artefact.
 *
 * ⭐ THE INTENT IS PRESERVED, NOT WEAKENED. The assertion existed to answer
 *    "am I testing the build I think I am". Comparing the SOURCE's declared
 *    version to the version WordPress reports for the RUNNING theme answers
 *    that better than a literal did: it catches a `style.css` that disagrees
 *    with the installed theme — a half-extracted or stale deploy — which is
 *    the failure the literal was standing in for. And it never goes stale.
 */
$c396_style_head = (string) file_get_contents( get_template_directory() . '/style.css', false, null, 0, 600 );
preg_match( '/^Version:\s*(\S+)/m', $c396_style_head, $c396_v );
c396_ok(
	'0.5 style.css declares the same version WordPress reports for the running theme',
	isset( $c396_v[1] ) && $c396_v[1] === (string) wp_get_theme()->get( 'Version' ),
	( isset( $c396_v[1] ) ? $c396_v[1] : '(none)' ) . ' vs ' . wp_get_theme()->get( 'Version' )
);
