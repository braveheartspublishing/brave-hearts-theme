...
/*
 * ═══════════════════════════════════════════════════════════════════════════
 * ⛔⛔ 2.5 — CORRECTED 1.19.399. THIS ASSERTION WAS A PERMANENT FAILURE.
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ⛔ WHAT IT WAS: `c397_ok( '2.5 the theme reports 1.19.397',
 *                          '1.19.397' === $theme_version, $theme_version );`
 *
 * ⛔ WHY IT WAS WRONG, AND IT IS WORTH BEING PRECISE ABOUT THE FAILURE MODE.
 *    The assertion is not "did the 397 work ship" — it is an EQUALITY on the
 *    theme version, so it goes red on 1.19.398, on this build, and on every
 *    build that will ever follow. It began failing the moment the next release
 *    was stamped and would have failed forever. A suite line that can never
 *    pass again teaches the reader to expect a red line, which is the one
 *    thing a regression suite must never do: the next REAL regression arrives
 *    into a fail list that already has a line in it.
 *
 * ⭐ FOUND BY `commerce-cx` during CYCLE179-CX-BUILD-398's staging
 *    suite run, recorded as finding (1) in that build's lock, and deliberately
 *    NOT fixed there — it is this file, this desk owns it, and the one-line
 *    change was `chief-of-staff`'s to call. It was called in the CYCLE179-LD-BUILD-399
 *    brief. ⭐ That is the correct sequence and it is recorded because it
 *    worked: the finding crossed a desk boundary as a finding, not as a
 *    silent edit to another lane's file.
 *
 * ⭐ WHAT IT IS NOW: `version_compare( $theme_version, '1.19.397', '>=' )`.
 *    ⛔ THE ASSERTION IS NOT WEAKENED INTO MEANINGLESSNESS — it still fails on
 *    any environment serving a theme older than the release this suite tests,
 *    which is the only thing it was ever able to prove. It stops failing for
 *    the sole reason that time passed.
 *
 * ⛔ THE OTHER 397 ASSERTIONS ARE UNTOUCHED. Every one of them tests actual
 *    397 BEHAVIOUR — the .min.css swap, the stamper's hook priority, the
 *    mtime suffix — and behaviour assertions are supposed to keep running
 *    against later builds. Only the version pin was time-bombed.
 */
c397_ok(
	'2.5 the theme reports 1.19.397 or later',
	version_compare( $theme_version, '1.19.397', '>=' ),
	$theme_version
);
