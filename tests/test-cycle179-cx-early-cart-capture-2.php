...
/*
 * ⛔⛔ §2.8 WAS PINNING THE PRE-FIX EXPRESSION AND HAS BEEN REPOINTED AT THE
 *     LIVE CODE. CORRECTED 2026-09-07, theme 1.19.393, `CYCLE179-CX-BUILD-391-1`,
 *     commerce-cx, on `lead-developer`'s finding
 *     (`ANDREW-REVIEW` 2026-09-07, `BUILD-392/cx-2.8-drawer-proof.md`).
 *
 * ⛔ SUPERSEDED, PRESERVED SO THE MOVEMENT STAYS VISIBLE:
 *       ~~strpos( $ecc2_js_src, 'cfg.isCart ? null' ) !== false~~
 *
 * ⭐ WHY IT WAS WRONG, WHICH IS THE PART THAT MATTERS. §11.0b OF THIS SAME
 *    FILE asserts `false === strpos( $ecc2_js_src, 'cfg.isCart ?' )` — that
 *    the truthiness comparison is GONE, because `wp_localize_script()`
 *    stringifies `0` and `"0"` is truthy in JavaScript. §2.8 required the very
 *    string §11.0b forbids. The two assertions could not both pass, and after
 *    the fix shipped, §2.8 was the one that had to move.
 *
 * ⚠ THE STRING SURVIVED IN A COMMENT, WHICH IS WHY THE FAILURE LOOKED
 *   INTERMITTENT. `cfg.isCart ? null` still appears on line 391 of
 *   `assets/js/early-cart-capture.js`, INSIDE the block comment that documents
 *   the fix. `bhp_ecc2_strip_js_comments()` removes it, so this assertion sees
 *   only real code — correctly.
 *
 * ⭐ THE CLAIM IS UNCHANGED: the drawer element is not looked up on /cart/, so
 *    the panel is never fetched there and a shopper is asked once, not twice
 *    behind an overlay. Only the instrument moved, from the old expression to
 *    the live one.
 */
bhp_ecc2_ok(
	'§2.8 ⛔ the drawer panel is NOT fetched on /cart/ (one ask per shopper, not two behind an overlay)',
	strpos( $ecc2_js_src, 'isCartSurface ? null' ) !== false
);
