 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * Design of record: a private spec outside this repo
 * (`CYCLE179-DES-PDP-COMPARE`), CONCEPT B. Andrew, seal 1208:
...
<section class="bhp-compare" aria-labelledby="bhp-compare-title">
  ...
  /*
   * ═════════════════════════════════════════════════════════════════════════
   * ⭐⭐⭐ 1.19.394 — THE "LOOK INSIDE" CUE MOVES UP, OUT OF THE FOOT OF THE
   *      COMPARISON AND UNDER THE PURCHASE BLOCK. `CYCLE179-CX-BUILD-394`, 5.
   * ═════════════════════════════════════════════════════════════════════════
   *
   * ⭐ ANDREW SIGNORE, 2026-09-07, verbatim. ⚠ RELAYED through the
   *    supervising session — NOT witnessed first-hand here (Standing
   *    Rules §9.2 rule 2): *"The look inside still needs to be brought up a
   *    little higher on the page."* "Still" — this is the second pass at it.
   *
   * ⛔ MEASURED ON STAGING 1.19.393 at a CSS viewport of 1536 x 830, so the
   *    distance is a number rather than an impression. The buy column ran:
   *
   *        .bhp-formats__note      (shipping)    y570 →  y594
   *        .bhp-product-guarantee  (30 day)      y602 →  y642
   *        .bhp-formats__amazon                  y651 →  y675
   *        .bhp-compare            (this)        y683 → y1632
   *        .bhp-compare__cue       (the cue)    y1518 → y1581   ⛔ 843px down
   *
   *    A parent deciding whether to buy had to scroll past the entire
   *    comparison table to be told the book can be looked inside — on a page
   *    whose gallery is sitting at the top of the very same screen.
   *
   * ⭐ IT IS NOW THE FIRST CHILD OF THIS SECTION, which puts it at y683: the
   *    line directly under the shipping and guarantee lines and directly above
   *    "One book, or all three". That is exactly the position the brief names.
   *
   * ⭐⭐ THE DOM MOVED, NOT THE PAINT ORDER. A CSS `order: -1` on this section
   *     would have produced the same picture in a third of the diff and left
   *     the reading order and the tab order pointing at the old place. The cue
   *     is a LINK. A link that is read last and painted first is a defect that
   *     only some people can see.
   *
   * ⛔ THE JUMP BEHAVIOUR IS UNTOUCHED, AND THAT WAS THE RISK. The `href`
   *    (`#bhp-look-inside-{key}`), the `data-bhp-gallery-jump` hook the
   *    gallery script binds to, the thumbnail chips, the video-conditional
   *    sub-line and the arrow are moved BYTE-FOR-BYTE. Nothing in this block
   *    was rewritten while it was being relocated.
   *
   * ⛔ THE COPY IS UNCHANGED. No new claim, no "we", no em dash, and the
   *    sub-line still names a flip-through video only when one actually
   *    resolved — a product claim on a purchase page, not a caption.
   */
  ?>
  <?php if ($bhp_cmp_cue): ?>
  ...
  <?php
  /*
   * ⛔⛔ THE "LOOK INSIDE" CUE USED TO BE HERE, AT THE FOOT OF THIS SECTION.
   *     IT MOVED TO THE TOP OF THIS SECTION AT 1.19.394 — see the head-note on
   *     the block, which now sits immediately after `<section class="bhp-compare">`.
   *     This comment is left at the old site so a reader who knows where the cue
   *     used to be is not left searching for it. ⛔ NOTHING ELSE MOVED: the
   *     `.bhp-compare__note` hardcover line below is exactly where it was.
   */
  ?>
